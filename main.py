...
def main():
    global sct_instance, current_display_image, screenshot_count, keep_running, monitor

    # Initialize directories and OpenCV window
    initialize_directories()
    setup_opencv_window()

    # Define the monitor dimensions
    monitor = auto_detect_monitor()

    # Start screen capture and keyboard listener
    with mss.mss() as sct:
        sct_instance = sct
        screenshot_count = 0
        keep_running = True
...
def initialize_directories():
    ...
    global UP_dir, DOWN_dir, LEFT_dir, RIGHT_dir
...
# Set up the OpenCV window for displaying screenshots.
def setup_opencv_window():
    cv2.namedWindow("Screenshot", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Screenshot", 960, 540)

# Capture and save a screenshot based on the key pressed.
...
# Automatically detect the primary monitor's dimensions.
